...
router.get('/welcome', (req, res) => {
  ...
  console.log('user account signed with token: ' + token)
...
router.get('/admin', (req, res) => {
  ...
  console.log('admin account signed with token: ' + token)
...
router.delete('/user', isAdmin, async (req, res) => {
  mongo.connect(process.env.DB_CONNECTION, function (err, db) {
    if (err) throw err //error handling
    //to get the database, username, collection details
    let dataBase = db.db('nkdatabase')
